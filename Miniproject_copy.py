...
def banner():
    ...
    print(cafe_name)
    print(border + '\n')
def printall_orders_couriers(table): 
    cursor = connection.cursor()
    ...
    if table == 'Orders': #print separate
        for row in rows:
            print(f'Order ID: {str(row[0])}, Order status: {row[1]}, Courier ID: {row[2]}, Customer Email: {row[3]}, Customer Address: {row[4]}, Customer contact: {row[5]}, Ordered Items: {row[6]}, Total Items Ordered: {row[7]}, Total cost {row[8]}')

    choice = input('Press 0 to return to main menu')
    if choice == '0':
        main_menu_input()
def print_products_each (type):

    cursor.execute( f'SELECT * FROM Products WHERE prod_type = "{column}"')
    option = cursor.fetchall()
    for row in option: 
        print(f'Product ID: {str(row[0])}, Product Name: {row[1]}, Unit: {row[3]}, Price: {row[4]}')
